import { json } from "../../server/utils";
import { db } from "../../server/db";
import type { Router } from "../../server/router";
import { auditLog } from "../../server/audit-logger";
import { randomBytes } from "node:crypto"; // For generating IDs
...
export function registerTAPlannerExtensionRoutes(router: Router) {
	...
	router.post("/api/ta-plans", async (req, _params, auth) => {
		...
		try {
			const id = generateId("tap"); // Use helper for ID generation
			db.query(`
				INSERT INTO ta_plans (
					...
					validation_status, status, created_by, details_json
				)
				VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', ?, ?)
			`).run(
				...
				'valid', auth.userId, JSON.stringify({ map_screenshot_base64: body.map_screenshot_base64 })
			);
	...
	// GET /api/ta-planner/tdok-sketches - List all TDOK sketches
	router.get("/api/ta-planner/tdok-sketches", async (_req, _params, auth) => {
        if (!auth) return json({ error: "Unauthorized" }, 401);
