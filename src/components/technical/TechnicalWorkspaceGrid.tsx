...
type WorkspaceGridCellProps = {
  ...
  registerCellSave?: (
    cellIndex: number,
    entry: { dirty: boolean; save: () => Promise<boolean> } | null,
  ) => void;
  ...
  useEffect(() => {
    ...
    const path =
      t?.type === "file"
        ? t.path
        : ((
            panelDock.tabs.find((x) => x.type === "file") as
...
export function TechnicalWorkspaceGrid({
  ...
  const cellSaveRegistry = useRef(
    new Map<number, { dirty: boolean; save: () => Promise<boolean> }>(),
  );
                ...
                <Fragment key={`wcell-${cellIndex}`}>
                  ...
                  >
                    <WorkspaceGridCell
                      ...
                      onReportFocused={onFocusedReport}
                      onCursor={(line, col) => {
                        if (focusedCell === cellIndex)
                          onFocusedCursor(line, col);
                      }}
                      editorRef={
                      ...
                      fileActions={fileActions}
                      onAddTool={(tab) =>
                        onOpenToolPanelForCell(cellIndex, tab)
                      }
                      onOpenWorkspace={onOpenWorkspace}
                      workspaceDockActions={workspaceDockActions}
                      wordWrap={wordWrap}
